...
<cfquery name="CustomerQuery" datasource="NitobiTestDB">
      ...
      SELECT TOP #TotalRecordsToRetrieve# * FROM tblMDOrders ORDER BY #SortColumn# #SortDirection#
...
<cfoutput query="CustomerQuery">
	...
	<cfif CurrentRecord GREATER THAN StartRecordIndex >
		#EBAGetHandler_CreateNewRecord(OrderID)#
			...
			#EBAGetHandler_DefineRecordFieldValue("Quantity", Quantity)#
			#EBAGetHandler_DefineRecordFieldValue("OrderDate", OrderDate)#
			#EBAGetHandler_DefineRecordFieldValue("ShippedDate", ShippedDate)#
