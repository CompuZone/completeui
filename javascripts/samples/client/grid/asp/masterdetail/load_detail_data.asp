...
<%
	...
	DefaultOrderByColumn = "OrderDate"
	...
	if len(request.querystring("CustomerID")) > 0 then
		WhereClause = " WHERE CustomerID LIKE '" & request.querystring("CustomerID") & "' "
	end if
	...
	countRecordSet = objConn.execute("SELECT COUNT(*) FROM " & MyTableName & WhereClause)
	dim MaxRecords
	...
	if MaxRecords > StartRecordIndex then
		...
		dim RecordSet
		Set RecordSet = objConn.execute("SELECT * FROM (SELECT TOP " & PageSize & " * FROM (SELECT TOP " & MaxRecords &  "  * FROM " & MyTableName  & WhereClause & " ORDER BY " & SortColumn & " " & SortDirection & ") ORDER BY " & SortColumn & " " & ReverseDirection & ") ORDER BY " & SortColumn & " " & SortDirection)
	...
	EBAGetHandler_DefineField("Quantity")	
	EBAGetHandler_DefineField("OrderDate")	
	EBAGetHandler_DefineField("ShippedDate") 
	...
	if MaxRecords > StartRecordIndex then
		do while not RecordSet.eof
			...
			EBAGetHandler_CreateNewRecord(RecordSet("OrderID"))
				...
				EBAGetHandler_DefineRecordFieldValue "Quantity", RecordSet("Quantity")
