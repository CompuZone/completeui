...
if cdbl(pageSize) < 1 then
	pageSize=10
end if

dim ordinalStart ' tells us where in the table to start returning records from'
ordinalStart = 0
ordinalStart = cdbl(request.querystring("start"))
if cdbl(ordinalStart) < 1 then
	ordinalStart=0
end if
...
EBAGetHandler_DefineField("ContactID")
...
newQuery = "SELECT * FROM (SELECT TOP " & pagesize & " * from (SELECT TOP " & maxRecords &  "  * FROM tblCustomers order by " & sortcolumn & " " & sortdirection & " ) order by " & sortcolumn & " " & reversedirection & " ) order by " & sortcolumn & " " & sortdirection
...
Do While (Not RecordSet.eof)
	...
	EBAGetHandler_DefineRecordFieldValue "ContactID", RecordSet("ContactID")
