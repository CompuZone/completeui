...
class EBASaveHandler
{
	...
	function CompleteSave()
	{
		if( $this->m_ErrorMessage != "" )
		{
			print str_replace("<root ", "<root error=\"".strtr($this->m_ErrorMessage,$this->m_TranscodeList)."\" ", $GLOBALS["HTTP_RAW_POST_DATA"]);
		}
		else
		{ 
	...
	function ReturnInsertField($RecordNumber, $FieldName = "")
	{
		...
		if (($index !== FALSE) || ($FieldName == ""))
		{
			...
			if (strlen($FieldName) > 0)
			{
				...
				if (($this->FieldsSet[$index] == "k") || ($this->FieldsSet[$index] == "i"))
				$EBASearchField = " ".$this->FieldsSet[$index] ."=\"";
			}
