...
<%
	...
	/**************************************************************8
		...
		We need to create:
		C:\Program Files\Nitobi\Grid V3.X\Jsp Trial\samples\masterdetail\data\MasterDetailDB.mdb
		...
		getServletPath()  ---> [samples\masterdetail] (get.jsp is removed)
		hard code string ---> [\data\MasterDetailDB.mdb]
	...
	*/
	...
	String path = config.getServletContext().getRealPath("/") + pathAfterContext + "\\data\\MasterDetailDB.mdb";
	...
	String newQuery = "SELECT * FROM ( SELECT TOP " + pageSize + " * FROM (SELECT TOP " + (pageSize+ordinalStart)+
					  " * FROM tblOrders " + whereClause + " ORDER BY " + sortColumn + " " + sortDirection +
	...
	myGetHandler.defineField("Quantity");
	myGetHandler.defineField("OrderDate");
	myGetHandler.defineField("ShippedDate");
	...
	{				
			...
			curRecord=myGetHandler.createNewRecord(rs.getString("OrderID"));
		
			curRecord.setField("ProductName",	rs.getString("ProductName"));	
			curRecord.setField("ProductPrice",	rs.getString("ProductPrice"));		
			curRecord.setField("Quantity",	rs.getString("Quantity"));
			curRecord.setField("OrderDate",		rs.getString("OrderDate"));
			curRecord.setField("ShippedDate",	rs.getString("ShippedDate"));		
