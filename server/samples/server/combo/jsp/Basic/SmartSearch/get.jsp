...
<%
    ...
    We need to create:
    	C:\Program Files\eBusiness Applications\Combo 3.X\Jsp Trial\samples\Basic\SmartSearch\data\folders.mdb
	...
	The above breakdown maps to the following:
		...
		getServletPath()  ---> [samples\Basic\SmartSearch] (get.jsp is removed)
	    hard code string ---> [\data\folders.mdb]
    ...
    */
	...
	path = path+pathAfterContext+"\\data\\folders.mdb";
	...
	String newQuery = "SELECT TOP " + pageSize + " * FROM tblFolderInfo WHERE FolderAbsolute LIKE '%" + searchSubString + "%' ORDER BY AccessAttempts DESC";	

	ResultSet rs = st.executeQuery(newQuery);
	
	GetHandler myGetHandler = new GetHandler(response,out);	
	
	myGetHandler.defineField("FolderName");
		
	// loop through the ResultSet from the Database and set values to myGetHandler
	Record curRecord;
	while ( rs.next() ) 
	{		
		curRecord=myGetHandler.createNewRecord(rs.getString("FolderID"));
		
		curRecord.setField("FolderName",rs.getString("FolderAbsolute"));
