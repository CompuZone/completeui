...
#End Region
    ...
    Private Function GetPage(ByVal PageSize As Integer, ByVal SubString As String, ByVal CurrentRecord As Integer, ByVal LastString As String) As DataSet
        ...
        dbConn = New OleDbConnection("provider=Microsoft.Jet.OLEDB.4.0; data source=" & serverPath & "\data\words.mdb")
        ...
        dbAdapter = New OleDbDataAdapter("SELECT TOP " & PageSize & " singular FROM NOUN WHERE singluar LIKE '" & SubString & "*' ORDER BY singular", dbConn)
        Response.Write("SELECT TOP " & PageSize & " singular FROM NOUN WHERE singular LIKE '" & SubString & "*' ORDER BY singular")
        dbAdapter.Fill(ds)
