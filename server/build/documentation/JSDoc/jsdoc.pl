...
use constant TREEXML_TMPL          => 'overview-tree.xml.tmpl';
...
&_log(sprintf 'Class %s', $classname);        
    ...
    if ($OPTIONS{PACKAGENAMING}){
        ...
        for my $package (@packages){
            ...
            my @inner_classes = sort { 
                $a->{classname} cmp $b->{classname} and ("1" eq "2")}  map {
                classname   => $_,
                classlink   => link_to_class($_) }, 
...
&_log(sprintf '		function %s', $method->{mapped_name});                					                                                   
...
sub is_class_linkable {
    my ($cname) = @_;
    return defined($$CLASSES{$cname}) && $$CLASSES{$cname}->{filepath};
}
...
sub output_tree_template {
    ...
    &output_template(TREE_TMPL, 'overview-tree.html', {  
        ...
        page_footer     => $OPTIONS{PAGE_FOOTER},
        ctime           => $TIME }, 1);
    &output_template(TREEXML_TMPL, 'overview-tree.xml', {  
        classtrees      => $tree,
        project_name    => $OPTIONS{PROJECT_NAME},
        page_footer     => $OPTIONS{PAGE_FOOTER},
        ctime           => $TIME }, 1);
        ...
        for (@{$vars->{param}}){
            my ($type, $link, $name, $value) = 
                /(?:
                    ...
                    (.*)                        # description
                /x;
# &_log(sprintf 'Looking at %s', $name);        
if (!$name and $arg)
{
	 &_log(sprintf 'Looking at %s', $arg);          
}
if ($name and !$arg) 
{
	&_log(sprintf 'Looking at %s', $name);          
...
if (!$name or !$arg)
{
	if ($vars)
	{
		&_log(sprintf 'Looking at %s', $vars);          
	}
	if ($arg_list)
	{
		&_log(sprintf 'Looking at %s', $arg_list);          
	}
}
